...
var bordg = []
var bordd = []
var used_cells = (preload("res://essai.tscn").instance()).get_child(0).get_used_cells()
...
func _ready():
	...
	for i in range(taille*taillem):
		abe.append(preload("res://bon.tscn").instance())
		abe.append(preload("res://bon.tscn").instance())
		abe.append(preload("res://roche.tscn").instance())
		spawn()
		spawn()
		spawn()
		#if (rand_range(1,2)==1):
...
func spawn():
	...
	add_child(abr)
	abr.global_translate(Vector2(y2*64,y1*64))
...
func ajout(i,x,y,tab):
	tab.append(preload("res://essai.tscn").instance())
	chill = Node2D.new()
	chill = tab[i]
	add_child(chill)
